def reward_function(params):
  ...
  center_marker_close2 = 0.15 * track_width
  ...
  # Track Segments
  # 1 Abbey / Starting Corner
  if (car_x > 0.7 and car_x < 2.4) and (car_y > 0.2 and car_y < 1.8):
    if not on_left_side and (abs_steering > 15.0) and (speed > 3.5):
      reward += 10.0
    else:
      reward -= 10.0

  # 2 Hamilton Straight / Starting Sprint
  if (car_x > 0.3 and car_x < 2.0) and (car_y > 1.8 and car_y < 2.5):
    if not on_left_side and (distance_from_border >= 0.04) and (speed > 3.5 ):
      reward += 12.5
    else:
      reward -= 10.0
  ...
  if (car_x > 0.4 and car_x < 1.8) and (car_y > 3.6 and car_y < 5.2):
    if not on_left_side and (distance_from_center <= center_marker_close2):
      reward += 12.5
    else:
      reward -= 10.0
  ...
  if (car_x > 1.8 and car_x < 3.0) and (car_y > 3.7 and car_y < 5.2):
    if not on_left_side and (speed > 3.5):
      reward += 10.0
    else:
      reward -= 10.0

  # 5 Becketts / The S-track
  if (car_x > 3.2 and car_x < 4.8) and (car_y > 2.3 and car_y < 5.1):
    if not on_left_side and (distance_from_border >= 0.04) and (speed > 3.3):
      reward += 12.5
    else:
      reward -= 10.0
  ...
  if (car_x > 4.8 and car_x < 5.8) and (car_y > 2.2 and car_y < 3.6):
    if not on_left_side and (distance_from_center <=  center_marker_close) and (speed > 3):
      reward += 12.5
    else:
      reward -= 10.0

  # 7， 8 Vale / Entering U-turn
  ...
  # 9 Club / Exiting U-turn
  if (car_x > 6.2 and car_x < 7.8) and (car_y > 0.3 and car_y < 2.0):
    if not on_left_side and (speed > 3) and (distance_from_border >= 0.03):
      reward += 10.0
    else:
      reward -= 10.0

  # 10 Wellington Straight / Sprint to Finish Line
  if (car_x > 2.2 and car_x < 6.1) and (car_y > 0.35 and car_y < 1.6):
    if not on_left_side and (distance_from_border >= 0.04) and (speed > 4):
      reward += 12.5
    else:
      reward -= 10.0
